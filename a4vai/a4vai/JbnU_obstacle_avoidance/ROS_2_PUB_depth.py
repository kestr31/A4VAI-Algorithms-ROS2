...
def publish_depth_image(client, left_img_pub, bridge_d):
    ...
    responses = client.simGetImages([airsim.ImageRequest("depth", airsim.ImageType.DepthPerspective, True, False)])
    
    if len(responses) == 0:
        # No image data received, fill with random values
        depth_img_in_meters = np.random.uniform(low=0.0, high=10.0, size=(84, 84)).astype(np.float32)
    else:
        response = responses[0]
        
        try:
            depth_img_in_meters = airsim.list_to_2d_float_array(response.image_data_float, 84, 84)
        except ValueError:
        ...
        if np.isnan(depth_img_in_meters).any() or np.isinf(depth_img_in_meters).any():
            depth_img_in_meters = np.ones((84, 84), dtype=np.float32) * 50.0
        
    depth_8bit_lerped = depth_img_in_meters.reshape(84, 84, 1)
